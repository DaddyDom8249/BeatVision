...
function normalizeCreativeMatchScore(value: unknown): number { const n = Number(value); if (!Number.isFinite(n)) return 0; const normalized = n <= 1 ? n : n / 100; return Math.max(0, Math.min(1, Math.round(normalized * 1000) / 1000)); }
...
if(!projectId) return new Response(JSON.stringify({error:'project_id is required.'}),{status:400,headers:{...corsHeaders(req),'Content-Type':'application/json'}});
