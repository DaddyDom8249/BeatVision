...
export type MotionJob = {
  job_id:string; project_id:string; stage:'motion'; provider:string; provider_job_id:string|null; status:MotionJobStatus;
  retry_count:number; max_retries:number; idempotency_key:string; created_at:string; updated_at:string;
  error:string|null; output:Record<string,unknown>|null; input:Record<string,unknown>; events:Array<Record<string,unknown>>;
};
...
export class BeatVisionMotionJob {
  ...
  async tick(job:MotionJob){
    if(!job.provider_job_id){
      ...
      if(!response.ok)throw new Error(`Pixazo submit ${response.status}: ${String(data?.message||data?.error||text).slice(0,1200)}`);
    ...
    if(['ERROR','FAILED','CANCELLED'].includes(status)){job.status='failed';job.error=String(data?.error||`Provider job ${status}`);await this.event(job,'provider_failed',{provider_job_id:job.provider_job_id,error:job.error});await this.save(job);return;}
  ...
  async fetch(request:Request):Promise<Response>{
    ...
    const input=body?.input&&typeof body.input==='object'?body.input:{};const job:MotionJob={job_id:String(body.job_id||crypto.randomUUID()),project_id:projectId,stage:'motion',provider:String(body.provider||'pixazo-ltx'),provider_job_id:null,status:'queued',retry_count:0,max_retries:Math.max(0,Math.min(Number(body.max_retries)||MAX_RETRIES,8)),idempotency_key:key,created_at:now(),updated_at:now(),error:null,output:null,input,events:[]};
