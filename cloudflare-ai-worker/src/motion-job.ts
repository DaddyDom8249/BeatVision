...
export type MotionJob = { job_id:string; project_id:string; stage:'motion'; provider:string; provider_job_id:string|null; status:MotionJobStatus; retry_count:number; poll_count:number; max_retries:number; max_polls:number; idempotency_key:string; created_at:string; updated_at:string; error:string|null; output:Record<string,unknown>|null; input:Record<string,unknown>; events:Array<Record<string,unknown>> };
const now=()=>new Date().toISOString(); const MAX_RETRIES=3; const MAX_POLLS=120;
const retryable=(s:string)=>/429|500|502|503|504|temporarily unavailable|rate limit|too many requests|timed out|timeout|aborted/i.test(s);
...
const durationValue=(value:unknown)=>{const n=Number(value);return Number.isFinite(n)?Math.max(1,Math.min(20,Math.round(n))):4;};
...
export class BeatVisionMotionJob {
  ...
  async tick(job:MotionJob){
    if(!job.provider_job_id){
      ...
      const input={prompt:String(job.input.prompt),image_url:String(job.input.image_url),duration:durationValue(job.input.duration),resolution:job.input.resolution==='1080p'?'1080p':'720p'};
      job.status='running';await this.save(job);
      const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),45000);let response:Response;try{response=await fetch(this.env.PIXAZO_LTX_ENDPOINT||'https://gateway.pixazo.ai/ltx-2-5-lite/v1/image-to-video',{method:'POST',headers:{'Content-Type':'application/json','Ocp-Apim-Subscription-Key':this.env.PIXAZO_API_KEY},body:JSON.stringify(input),signal:controller.signal});}finally{clearTimeout(timer);}
      ...
      job.provider_job_id=String(data.request_id);job.status='polling';job.retry_count=0;job.poll_count=0;await this.event(job,'provider_submitted',{provider_job_id:job.provider_job_id});await this.state.storage.setAlarm(Date.now()+7000);return;
    }
    if(job.poll_count>=job.max_polls){job.status='failed';job.error='Provider polling exceeded the durable job limit.';await this.event(job,'poll_limit_reached',{poll_count:job.poll_count});return;}
    job.poll_count++;await this.save(job);
    ...
    job.status='polling';await this.save(job);await this.state.storage.setAlarm(Date.now()+7000);
  }
  async fetch(request:Request):Promise<Response>{
    ...
    const job:MotionJob={job_id:String(body.job_id||crypto.randomUUID()),project_id:projectId,stage:'motion',provider:String(body.provider||'pixazo-ltx-2-5-lite'),provider_job_id:null,status:'queued',retry_count:0,poll_count:0,max_retries:Math.max(0,Math.min(Number(body.max_retries)||MAX_RETRIES,8)),max_polls:Math.max(1,Math.min(Number(body.max_polls)||MAX_POLLS,240)),idempotency_key:key,created_at:now(),updated_at:now(),error:null,output:null,input,events:[]};
