...
const arenaSource=read('worker/src/arena-entry.ts');
if(arenaSource.includes('EXTERNAL_LANGUAGE_PROVIDER_URL')||arenaSource.includes('EXTERNAL_LANGUAGE_PROVIDER_TOKEN')||arenaSource.includes('EXTERNAL_LANGUAGE_PROVIDER_MODEL'))throw new Error('External language provider remains in the active Arena language path.');
if(!arenaSource.includes("ai.run('@cf/meta/llama-3.1-8b-instruct-fp8'"))throw new Error('Workers AI language generation is missing from the active Arena path.');
...
if(!wrangler.includes('[ai]')||!wrangler.includes('binding = "AI"'))throw new Error('Workers AI binding is missing from the Arena Worker.');
