...
async function languageGenerate(r: Request, e: any, body: any, requestId: string) {
  if (body?.contract_version !== CONTRACT || body?.operation !== 'generate') {
    return json(r, e, { ok: false, contract_version: CONTRACT, capability: 'language', status: 'contract_mismatch', request_id: requestId, error: 'Expected BeatVision contract 1.1 language generate.' }, 400);
  }

  const payload = body?.payload || {};
  const prompt = clip(payload?.prompt, 30000);
  if (!prompt) {
    return json(r, e, { ok: false, contract_version: CONTRACT, capability: 'language', status: 'invalid_input', request_id: requestId, error: 'Language generation prompt is required.' }, 400);
  }

  const ai = e.AI;
  if (!ai || typeof ai.run !== 'function') {
    return json(r, e, { ok: false, contract_version: CONTRACT, capability: 'language', provider: 'workers-ai', status: 'provider_unavailable', request_id: requestId, error: 'Workers AI binding is not configured.' }, 503);
  }
  ...
  try {
    const result = await ai.run('@cf/meta/llama-3.1-8b-instruct-fp8', {
      prompt: [
        ...
        'Preserve creative intent, continuity, song timing, and production usefulness.',
    ...
    if (String(payload?.mode || '') === 'storyboard') {
      ...
      return json(r, e, {
        ok: true, contract_version: CONTRACT, capability: 'language', provider: 'workers-ai',
        model: '@cf/meta/llama-3.1-8b-instruct-fp8', status: 'generated',
        latency_ms: Date.now() - started, request_id: requestId,
        result: toStoryboard(normalized), coverage: normalized.coverage
      });
    }
    ...
    return json(r, e, {
      ok: true, contract_version: CONTRACT, capability: 'language', provider: 'workers-ai',
      model: '@cf/meta/llama-3.1-8b-instruct-fp8', status: 'generated',
      latency_ms: Date.now() - started, request_id: requestId, result: parsed
    });
  } catch (error) {
    const timed = error instanceof Error && error.name === 'AbortError';
    return json(r, e, {
      ok: false, contract_version: CONTRACT, capability: 'language', provider: 'workers-ai',
      status: timed ? 'provider_timeout' : 'provider_error',
      latency_ms: Date.now() - started, request_id: requestId,
      error: timed ? 'Workers AI language generation timed out.' : String(error instanceof Error ? error.message : error).slice(0, 1600)
    }, timed ? 504 : 502);
  }
}
